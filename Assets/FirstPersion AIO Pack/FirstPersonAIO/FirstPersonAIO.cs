...
public class FirstPersonAIO : MonoBehaviour
{
    ...
    public void RotateCamera(Vector2 Rotation, bool Snap)
    {
        ...
        if (Snap)
        { followAngles = Rotation; targetAngles = Rotation; }
        else
    ...
    private void OnCollisionEnter(Collision CollisionData)
    {
        for (int i = 0; i < CollisionData.contactCount; i++)
        {
            ...
            if (CollisionData.GetContact(i).point.y < transform.position.y - ((capsule.height / 2) - capsule.radius * 0.95f))
            {
                ...
                {
                    if (a < 5.1f)
                    { advanced.isTouchingFlat = true; advanced.isTouchingWalkable = true; }
                    else if (a < advanced.maxSlopeAngle + 0.1f)
                    { advanced.isTouchingWalkable = true; /* IsGrounded = true; */}
                    else if (a < 90)
    ...
    private void OnCollisionExit(Collision CollisionData)
    {
        IsGrounded = false;
        if (advanced.maxSlopeAngle > 0)
        { advanced.curntGroundNormal = Vector3.up; advanced.lastKnownSlopeAngle = 0; advanced.isTouchingWalkable = false; advanced.isTouchingUpright = false; }
...
public class FPAIO_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (t.controllerPauseState)
        { GUILayout.Label("<b><color=#B40404>Controller Paused</color></b>", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, richText = true, fontSize = 16 }); }
        ...
        if (t.cameraInputMethod == FirstPersonAIO.CameraInputMethod.Traditional)
        ...
        if (!t.playerCamera)
        { EditorGUILayout.HelpBox("A Camera is required for operation.", MessageType.Error); }
        ...
        if (t.autoCrosshair)
        { EditorGUI.indentLevel++; EditorGUILayout.BeginHorizontal(); EditorGUILayout.PrefixLabel(new GUIContent("Crosshair", "Sprite to use as a crosshair.")); t.Crosshair = (Sprite)EditorGUILayout.ObjectField(t.Crosshair, typeof(Sprite), false); EditorGUILayout.EndHorizontal(); EditorGUI.indentLevel--; }
        GUI.enabled = true;
        EditorGUILayout.Space();
        #endregion
        ...
        GUI.enabled = t.playerCanMove && t.canJump;
        ...
        if (showAdvanced)
        {
            ...
            GUI.enabled = t.playerCanMove && t.useStamina;
            EditorGUI.indentLevel++;
            ...
            if (t.advanced.FOVKickAmount > 0)
            { t.advanced.changeTime = EditorGUILayout.Slider(new GUIContent("FOV Change Time", "Determines the speed of the FOV kick"), t.advanced.changeTime, 0.1f, 2); }
            EditorGUILayout.Space();
            ...
            if (t.advanced.maxSlopeAngle > 0)
            { EditorGUILayout.HelpBox("For slops with angles greater than 55° should have a 90° (upright) collider positioned at the bottom of the slope. With out this, the player will have trouble jumping while touching both ground and the slope.", MessageType.Info); }
            EditorGUILayout.Space();
        ...
        if (!t.head)
        { EditorGUILayout.HelpBox("A Head Transform is required for headbobbing.", MessageType.Error); }
        ...
        if (t.fsmode == FirstPersonAIO.FSMode.Static)
        {
            ...
            if (showStaticFS)
            {
                ...
                for (int i = 0; i < staticFS.arraySize; i++)
                {
                    ...
                    { this.t.footStepSounds.RemoveAt(i); }
        ...
        else
        {
            ...
            if (showWoodFS)
            {
                ...
                {
                    if (!t.dynamicFootstep.woodPhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < woodPhysMat.arraySize; i++)
                    {
                        ...
                        if (GUILayout.Button(new GUIContent("X", "Remove this Physic Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.woodPhysMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }


                    if (GUILayout.Button(new GUIContent("Add new Physic Material entry", "Add new Physic Material entry")))
                ...
                {
                    if (!t.dynamicFootstep.woodMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < woodMat.arraySize; i++)
                    {
                        ...
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.woodMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.woodMat.Add(null); }
                ...
                for (int i = 0; i < woodFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.woodClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.woodClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.woodClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
            ...
            if (showMetalFS)
            {
                ...
                if (t.dynamicFootstep.materialMode == FirstPersonAIO.DynamicFootStep.matMode.physicMaterial)
                {
                    if (!t.dynamicFootstep.metalAndGlassPhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                ...
                {
                    if (!t.dynamicFootstep.metalAndGlassMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < metalAndGlassMat.arraySize; i++)
                    {
                        ...
                        LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("", LS_ref.objectReferenceValue, typeof(Material), false);
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.metalAndGlassMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.metalAndGlassMat.Add(null); }
                ...
                for (int i = 0; i < metalFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.metalAndGlassClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.metalAndGlassClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.metalAndGlassClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
            ...
            if (showGrassFS)
            {
                ...
                if (t.dynamicFootstep.materialMode == FirstPersonAIO.DynamicFootStep.matMode.physicMaterial)
                {
                    if (!t.dynamicFootstep.grassPhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                ...
                {
                    if (!t.dynamicFootstep.grassMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < grassMat.arraySize; i++)
                    {
                        ...
                        LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("", LS_ref.objectReferenceValue, typeof(Material), false);
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.grassMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.grassMat.Add(null); }
                ...
                for (int i = 0; i < grassFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.grassClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.grassClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.grassClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
            ...
            if (showDirtFS)
            {
                ...
                if (t.dynamicFootstep.materialMode == FirstPersonAIO.DynamicFootStep.matMode.physicMaterial)
                {
                    if (!t.dynamicFootstep.dirtAndGravelPhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                ...
                {
                    if (!t.dynamicFootstep.dirtAndGravelMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < dirtAndGravelMat.arraySize; i++)
                    {
                        ...
                        LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("", LS_ref.objectReferenceValue, typeof(Material), false);
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.dirtAndGravelMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.dirtAndGravelMat.Add(null); }
                ...
                for (int i = 0; i < dirtFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.dirtAndGravelClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.dirtAndGravelClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.dirtAndGravelClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
            ...
            if (showConcreteFS)
            {
                ...
                if (t.dynamicFootstep.materialMode == FirstPersonAIO.DynamicFootStep.matMode.physicMaterial)
                {
                    if (!t.dynamicFootstep.rockAndConcretePhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                ...
                {
                    if (!t.dynamicFootstep.rockAndConcreteMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < rockAndConcreteMat.arraySize; i++)
                    {
                        ...
                        LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("", LS_ref.objectReferenceValue, typeof(Material), false);
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.rockAndConcreteMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.rockAndConcreteMat.Add(null); }
                ...
                for (int i = 0; i < concreteFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.rockAndConcreteClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.rockAndConcreteClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.rockAndConcreteClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
            ...
            if (showMudFS)
            {
                ...
                if (t.dynamicFootstep.materialMode == FirstPersonAIO.DynamicFootStep.matMode.physicMaterial)
                {
                    if (!t.dynamicFootstep.mudPhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                ...
                {
                    if (!t.dynamicFootstep.mudMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < mudMat.arraySize; i++)
                    {
                        ...
                        LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("", LS_ref.objectReferenceValue, typeof(Material), false);
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.mudMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.mudMat.Add(null); }
                ...
                for (int i = 0; i < mudFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.mudClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.mudClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.mudClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
            ...
            if (showCustomFS)
            {
                ...
                if (t.dynamicFootstep.materialMode == FirstPersonAIO.DynamicFootStep.matMode.physicMaterial)
                {
                    if (!t.dynamicFootstep.customPhysMat.Any())
                    { EditorGUILayout.HelpBox("At least one Physic Material must be assigned first.", MessageType.Warning); }
                ...
                {
                    if (!t.dynamicFootstep.customMat.Any())
                    { EditorGUILayout.HelpBox("At least one Material must be assigned first.", MessageType.Warning); }
                    ...
                    for (int i = 0; i < customMat.arraySize; i++)
                    {
                        ...
                        LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("", LS_ref.objectReferenceValue, typeof(Material), false);
                        if (GUILayout.Button(new GUIContent("X", "Remove this Material"), GUILayout.MaxWidth(20)))
                        { t.dynamicFootstep.customMat.RemoveAt(i); }
                        EditorGUILayout.EndHorizontal();
                    }
                    if (GUILayout.Button(new GUIContent("Add new Material entry", "Add new Material entry")))
                    { t.dynamicFootstep.customMat.Add(null); }
                ...
                for (int i = 0; i < customFS.arraySize; i++)
                {
                    ...
                    LS_ref.objectReferenceValue = EditorGUILayout.ObjectField("Clip " + (i + 1) + ":", LS_ref.objectReferenceValue, typeof(AudioClip), false);
                    if (GUILayout.Button(new GUIContent("X", "Remove this clip"), GUILayout.MaxWidth(20)))
                    { t.dynamicFootstep.customClipSet.RemoveAt(i); }
                    EditorGUILayout.EndHorizontal();
                }
                ...
                if (GUILayout.Button(new GUIContent("Add Clip", "Add new clip entry")))
                { t.dynamicFootstep.customClipSet.Add(null); }
                if (GUILayout.Button(new GUIContent("Remove All Clips", "Remove all clip entries")))
                { t.dynamicFootstep.customClipSet.Clear(); }
                EditorGUILayout.EndHorizontal();
                GUILayout.EndVertical();
        ...
        */
        ...
        if (!loadedAds)
        {
            if (GUILayout.Button("View Assets"))
            { loadedAds = true; DownloadImage("http://www.aedangraves.info/portfolio-site/MiscSite_content/Images/adTex1.png"); }
        }
        else
